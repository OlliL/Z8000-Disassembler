 ...
 *   Z8001 disassembler; requires all z8000_exec tables to be initialized
 *
...
#ifndef Z8K1_DASM
	"f",   "lt",  "le",  "ule",  "pe/ov",   "mi",  "eq/z",   "c/ult", "t",   "ge",  "gt",  "ugt",  "po/nov",  "pl",  "ne/nz",  "nc/uge"
#else
	"f",   "lt",  "le",  "ule",  "pe",      "mi",  "z",      "c",     "t",   "ge",  "gt",  "ugt",  "po",      "pl",  "nz",     "nc"
#endif
};

static const char *const flg[16] = {
#ifndef Z8K1_DASM
	"",    "p/v",  "s",   "p/v,s",   "z",   "p/v,z",  "s,z",  "p/v,s,z", "c",   "p/v,c","s,c", "p/v,s,c", "z,c", "p/v,z,c","s,z,c","p/v,s,z,c"
#else
	"",    "p",    "s",   "p,s",     "z",   "p,z",    "s,z",  "p,s,z",   "c",   "p,c","s,c",   "p,s,c",   "z,c", "p,z,c",  "s,z,c","p,s,z,c"
#endif
};
...
offs_t z8000_dasm(char *buff, offs_t pc, const UINT8 *oprom, const UINT8 *opram, Z8000_exec* pZ8000_exec)
{
	offs_t new_pc = pc;
        int  i, tmp;
	char *dst = buff;
	...
	switch (pc)
	{
		...
		case 0x0002:
			dst += sprintf(dst, ".word   #%%%04x ;RST FCW", w[0]);
            		break;
		case 0x0004:
			dst += sprintf(dst, ".word   #%%%04x ;RST SEG", w[0]);
            		break;
		case 0x0006:
		...
		default:
			...
			while (*src)
			{
				if (*src == '%')
				{
					...
					case '^':
						src++;
                                                if(~n[7] & 0x08) dst += sprintf(dst, "r");                   // used for ld[di][r][b]
                                                break;
                                        case 'A':
                                                /* address */
                                                src++;  
						i = *src++ - '0';
                                                if(w[i] & 0x8000) {
                                                   GET_OP(oprom, o->size, new_pc - pc);
                                                   new_pc += 2;
                                                   if(segdisp32hex) {
                                                      dst += sprintf(dst, "%%%04x",w[i++] & 0x7f00);
                                                      dst += sprintf(dst, "%04x", w[i]);
                                                   } else {
                                                      dst += sprintf(dst, "<<%%%x>>",(w[i++] & 0x7f00)>>8);
                                                      dst += sprintf(dst, "%%%04x", w[i]);
                                                   }
                                                } else {
                                                   if(segdisp32hex) 
                                                      dst += sprintf(dst, "|%%%04x%04x|",w[i] & 0xff00,w[i] & 0x00ff);
                                                   else
                                                      dst += sprintf(dst, "|<<%%%x>>%%%02x|",w[i]>>8, w[i] & 0xff);
                                                }
                                                break;
                                        case 'D':
                                                /* immediate in segmented variable position data words (CP|LD(B) addr,#data) */
                                                src++;
                                                switch (*src++) {
                                                        case 'b': /* imm8 (byte) */
                                                                i = *src++ - '0';
					...
					case 'R':
						...
						switch (tmp)
						{
							...
							case 0x030: dst += sprintf(dst, "sotirb"); break;
							case 0x038: dst += sprintf(dst, "soutib"); flags = DASMFLAG_STEP_OVER; break;
							...
							case 0x0b0: dst += sprintf(dst, "sotdrb"); break;
							case 0x0b8: dst += sprintf(dst, "soutdb"); flags = DASMFLAG_STEP_OVER; break;
							...
							case 0x120: dst += sprintf(dst, "outir  "); flags = DASMFLAG_STEP_OVER; break;
							case 0x128: dst += sprintf(dst, "outi  "); break;
							case 0x130: dst += sprintf(dst, "sotir "); break;
							case 0x138: dst += sprintf(dst, "souti "); flags = DASMFLAG_STEP_OVER; break;
							...
							case 0x1b0: dst += sprintf(dst, "sotdr "); break;
							case 0x1b8: dst += sprintf(dst, "soutd "); flags = DASMFLAG_STEP_OVER; break;
                            				default:    dst += sprintf(dst, "??????");
						}
                        			break;
                    			case 'a':
						/* address */
					...
					case 'i':
						...
						break;
                                        case 'n':
                                                /* ldm qualifier */
						src++;
                                                dst += sprintf(dst, "%d", 1+(n[7] & 0xf));
                                                break;
					case 'p':
						/* disp16 (pc relative) */
						src++;
						i = *src++ - '0';
						dst += sprintf(dst, "%%%04x", (new_pc + w[i]) & 0xffff);
						break;
					case 'r':
						/* register */
						src++;
